...
% title info
\title{\normalsize Efficient non-parametric inference on the causal effects of
  stochastic interventions in two-phased vaccine efficacy trials}
...
\institute{
  \begin{figure}[!htb]
    ...
    \begin{minipage}{.65\textwidth}
      ...
      WNAR student paper competition presentation
    \end{minipage}%
...
\begin{frame}[c]{Targeted minimum loss estimator}

\begin{center}
\begin{itemize}
  \itemsep10pt
  \item TML estimation algorithm updates initial estimators $g_n$ and
    $\overline{Q}_n$ so as to satisfy an arbitrary set of estimating equations.
  \item TML estimator built by updating initial estimates of outcome mechanism
    $\overline{Q}_n$ via a (logistic) tilting model, yielding
    \begin{equation*}\label{tmle}
      \Psi_n \coloneqq \Psi(P_n^{\star}) = \frac{1}{n} \sum_{i = 1}^n
        \overline{Q}_n^{\star}(d(A_i, L_i), L_i).
      \end{equation*}
  \item In this case, the efficient influence function (EIF) is
    \begin{equation*}
      D(P_0^X)(x) = H(a, l)({y - \overline{Q}(a, l)}) +
      \overline{Q}(d(a, l), l) - \Psi(P_0^X)
    \end{equation*}
  \item The auxiliary covariate $H(a,l)$ may be expressed
    \begin{equation*}
      H(a,l) = \I(a + \delta < u(l)) \frac{g_0(a - \delta \mid l)}
      {g_0(a \mid l)} + \I(a + \delta \geq u(l))
    \end{equation*}
\end{itemize}
\end{center}
