...
% title info
\title{\normalsize Stochastic-interventional causal effects to evaluate
  vaccine efficacy in clinical trials with outcome-dependent sampling}
%Characterizing immune correlates of protection in vaccine
  ...
  \item The HIV epidemic:
    \begin{itemize}
      ...
      \item HIV Vaccine Trials Network's (HVTN) \textit{505} trial evaluated a
        novel antibody boost vaccine for HIV-1~\citep{hammer2013efficacy}.
    \end{itemize}
  \item The COVID-19 \sout{epi}~\sout{pan}~endemic~\citep{antia2021transition}:
    \begin{itemize}
      \itemsep4pt
      \item \sout{270} \sout{331} \sout{619} \sout{643} \sout{668} 673 million
        total cases detected globally;
...
\begin{itemize}
  ...
  \item Outcome ($Y$): HIV infection status at week 28 of tiral.
  ...
  \item Cryopreserved peripheral blood mononuclear cells were stimulated with
    synthetic HIV peptide pools.
  \item A vaccine effective at preventing HIV acquisition would be a
  ...
  \item The study was halted on 22 April 2013 due to absence of vaccine
    ...
    infection endpoint of HIV infection between week 28 and month 24.
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[c]{Evaluating vaccines for HIV and COVID-19}

\begin{center}
\begin{itemize}
  \itemsep8pt
  \item \textit{505}: How would HIV-1 infection risk have differed had
    the boost vaccine modulated immune responses differently?
  \item \textit{COVE}: How would COVID-19 disease rate have differed for
    alternative vaccine-induced immune response profiles?
  \item \textbf{Question}: How can [HIV, COVID-19] vaccines be improved
    through the modulation of immune response profiles?
\end{itemize}
\end{center}
...
\begin{itemize}
  ...
  \item Two, interrelated goals of immune correlates analyses are to
    \begin{itemize}
      ...
      \item understand/dissect \textit{protective mechanisms} of vaccines.
    \end{itemize}
  \item If an immune correlate is shown to be reliably predictive of
    vaccine efficacy (VE), used as primary endpoint in later trials.
  \item Validated immune correlates may accelerate the approval of
    \begin{itemize}
      ...
      \item variant-adapted vaccines only via clinical immunobridging.
    \end{itemize}
\end{itemize}
\end{center}
...
\begin{itemize}
  \itemsep6pt
    \item Two-phase sampling designs~\citep{breslow2003large} (e.g.,
    \citet{prentice1986case}'s case-cohort) are standard practice for measuring
    candidate immune correlates~\citep{follmann2006augmented}.
    \item ``Phase 0'': random sampling from the target population.
    ...
    \item Phase 2: given baseline, vaccine, endpoint, select members of
      ...
      \begin{itemize}
        ...
        \item \textit{505}: phase-two sample with 100\% of HIV-1 cases and
          post-hoc matching of
          non-cases~\citep[$n = 189$ per][]{janes2017higher}.
        \item \textit{COVE}: pre-specified stratified random subcohort ($n
        \approx 1600$), plus all SARS-CoV-2 infection, COVID-19 disease
        endpoints.
      \end{itemize}
\end{itemize}
\end{center}
...
\begin{itemize}
  ...
  \item Complete (unobserved) data $X = (L, A, S, Y) \sim P_0^X \in
    ...
    \begin{itemize}
      \itemsep4pt
      \item $L$ (baseline covariates): sex, age, BMI, behavioral risk score,
      \item $A$ (treatment): randomized assignment to vaccine/placebo,
      \item $S$ (mediator): immune response profile for relevant markers,
      \item $Y$ (outcome): infection/disease endpoint at end of follow-up.
    \end{itemize}
  \item Observed data $O = (R, RX) = (L, A, RS, Y, R) \sim P_0 \in \mathcal{M}$.
    \begin{itemize}
      \itemsep4pt
      \item $R \in \{0,1\}$ is an indicator of inclusion in the phase-two
      sample.
      \item $g_{0,R} \coloneqq \pr(R = 1 \mid Y, L)$ must be \textit{known by
        design} or estimated.
...
\begin{frame}[standout]
  Causal Effects for ``Quantitative'' Variables
\end{frame}
...
\begin{itemize}
  \itemsep8pt
  \item Describe the manner in which $X$ is hypothetically generated using a
    structural causal model~\citep{pearl2009causality}:
    \begin{equation*}
  ...
  \item A \textit{static} (deterministic) intervention replaces $f_S$ with a
    specific value $s$ in its conditional support $S \mid A, L$.
  \item This requires specifying a particular value of $S$ \textit{a priori}
    under which to evaluate the outcome --- but what value?
\end{itemize}
\end{center}
...
\begin{itemize}
  ...
  and $S = s_0$, where $s_0 = 0$ can be shown to hold.
\end{itemize}
\end{center}
...
\hspace*{-1cm}\includegraphics[scale=0.41]{shift-5}
...
\hspace*{-1cm}\includegraphics[scale=0.41]{shift-6}
...
\hspace*{-1cm}\includegraphics[scale=0.41]{shift-7}
...
\begin{itemize}
  \itemsep6pt
  \item Consider a data structure: $(Y_s, s \in
    \mathcal{S})$~\citep{gill2001causal}.
...
\note{
  \begin{itemize}
    \item For HVTN 505, $\psi_{0,\delta}$ is the counterfactual risk of HIV
      infection, had the observed value of the immune response been altered
      under the rule $d(S,L)$ defining $g_{\delta}(\cdot \mid L)$.
  \end{itemize}
}

\footnotetext[1]{\citet{haneuse2013estimation} defined modified treatment
policies (MTPs).}
...
\begin{itemize}
  ...
  \item The efficient influence function of $\psi_{0, \delta}$ with respect to
    $\mathcal{M}$ is
    \begin{equation*}
      D^{\star}(P_0)(x) = \frac{g_{0,S}(d^{-1}(s,l) \mid l)}
...
\begin{itemize}
  ...
  \item \cite{rose2011targeted2sd} suggested inverse probability of censoring
    ...
    \begin{equation*}
      \lik(P_0^X)(O) = \frac{R}{g_{0,R}(Y, L)}\lik(P_0^X)(X)
    \end{equation*}
  \item When the sampling mechanism $g_{0,R}(Y,L)$ is known by design, this
    procedure yields a reasonably reliable estimator.
  \item When data-adaptive regression must be used --- that is, when
    $g_{0,R}(Y,L)$ is not known by design\footnotemark --- this is insufficient.
\end{itemize}
\end{center}
...
\begin{itemize}
  ...
  \item Then, the IPCW augmentation must be applied to the EIF\footnotemark:
    \begin{align*}
      D^{\star}(P_0^X)(o) = &\frac{r}{g_{0,R}(y, l)} D^{\star}(P_0^X)(x) -
        \left(1 - \frac{r}{g_{0,R}(y, l)}\right) \\
        &\E(D^{\star}(P_0^X)(x) \mid R = 1, Y = y, L = l).
    \end{align*}
  \item Expresses observed data EIF $D^{\star}(P_0^X)(o)$ via complete data
     EIF $D^{\star}(P_0^X)(x)$; inclusion of second term improves efficiency.
 \item An emergent multiple robustness property --- combinations of
   \begin{itemize}
       \item $\{g_{0,S}(S \mid L), \overline{Q}_{0,Y}(S,L)\}$, due to choice
       of target parameter;
       \item $\{g_{0,R}(Y, L), \E(D^{\star}(P^X_0)(x) \mid R = 1, Y, L)\}$, due
       to sampling.
   \end{itemize}
...
\begin{itemize}
  \itemsep6pt
  \item The expectation of the full data EIF $D^{\star}(P_0^X)(x)$, taken
    only over units selected by the sampling mechanism (i.e., $R = 1$).
\end{itemize}
}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.21]{sve_pseudoneutid50_simplified_talksonly}
\caption{
...
\begin{itemize}
  ...
  \item SVE predictions and TND-based VE estimates well-correlated, but
      SVE predictions are likely underestimates, since PsV nAb correlate is an
      \textit{imperfect} (i.e., partial) causal \textit{mediator} of VE.
    %\begin{itemize}
      %\itemsep2pt
...
\begin{itemize}
  \itemsep8pt
  \item We considered the case of $O = (L, A, RS, Y, R)$, but what about $O =
    (V, L, A, RS, Y, R)$ or $O = (L, A, Z, RS, Y, R)$?
    \begin{itemize}
      ...
      \item $Y$: symptomatic SARS-CoV-2 (or HIV) infection
      \item $R \coloneqq f(Y, L)$: selection into phase-two sample
    \end{itemize}
  \item And what about survival endpoints, $O = (L, A, RS, \Delta,
    \widetilde{T}, R)$?
    \begin{itemize}
      \item $\widetilde{T} = \min(T_F, T_C)$: possibly right-censored failure
        time.
      \item $\Delta = \mathbb{I}(T_F < T_C)$: indicator of failure
        endpoint occurrence.
      \item Could making $R$ a function of $\widetilde{T}$ improve sampling
        efficiency? (Considered for the Cox model by~\cite{borgan1995methods}.)
    \end{itemize}
\end{itemize}
\end{center}
...
\begin{itemize}
  ...
  %\item Stochastic interventions provide a framework for guiding novel policies
    %that induce changes to ``natural'' (treatment) patterns.
  \item Modified treatment policies help to formulate causal questions about
    \textit{realistic} interventions on quantitative variables.
  ...
  \item Open source software for the SVE approach is freely available and
    already in use for analyses of several large-scale VE trials.
\end{itemize}
\end{center}
