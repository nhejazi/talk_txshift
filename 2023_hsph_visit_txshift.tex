...
%\begin{frame}[c]{Causal Machine Learning}

%\begin{center}
%\begin{itemize}
  %\itemsep8pt
  %\item Continuous treatments (biomarker activity, drug dosage) --- how can
    %informative causal effect contrasts be formulated?
  %\item Novel (in)direct effects: characterize \textit{mechanistic} pathways.
  %\item Recent scientific results from these contributions:
    %\begin{itemize}
      %\itemsep4pt
      %\item Evaluating immune correlates via stochastic vaccine efficacy
        %parameters, with corrections for outcome-dependent sampling.
      %\item Causal mediation analysis with stochastic interventions: two new
        %effect definitions, with state-of-the-art efficient estimators.
      %\item Relaxed identification: (in)direct effects under unmeasured
        %confounding, weaker ``positivity'' conditions for total effects.
      %\item Causal transport: bridging VE across variants, fractional doses.
    %\end{itemize}
%\end{itemize}
%\end{center}
...
%\begin{frame}[c]{Reproducible Statistical Data Science}
...
%\begin{itemize}
  ...
  %%\item Recent contributions and collaborations:
    %%\begin{itemize}
      %%\itemsep4pt
      %%\item Targeted Learning in the \texttt{tlverse}: building an \texttt{R}
        %%package ecosystem that aligns statistical theory with software design.
      %%\item Co-design of statistical analysis suite for immune correlates
        %%studies, focusing on modularity and reproducibility.
    %%\end{itemize}
%\end{itemize}
%\end{center}
